            ...
            <div class="card">
                ...
                <form class="form-horizontal" action="{{ route('admin.discounts.store') }}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label class="col-sm-4 control-label">کاربر مربوط به تخفیف (اختیاری)</label>
                            <select class="form-control" name="users[]" id="users" aria-describedby="helpId" multiple>
                                <option value="null">همه کاربرها</option>
                                @foreach(\App\User::all() as $user)
                                    <option value="{{ $user->id }}">{{ $user->name }}</option>
                                @endforeach
                            </select>
                            <small id="helpId" class="text-muted">در صورت خالی بودن، همه کاربران لحاظ خواهند شد</small>
                        </div>
                        <div class="form-group">
                            <label for="inputPassword3" class="col-sm-4 control-label">محصول مربوطه (اختیاری)</label>
                            <select class="form-control" name="products[]" id="products" aria-describedby="helpId" multiple>
                                <option value="null">همه محصول</option>
                                @foreach(\App\Product::all() as $product)
                                    <option value="{{ $product->id }}">{{ $product->title }}</option>
                                @endforeach
                            </select>
                            <small id="helpId" class="text-muted">در صورت خالی بودن، همه محصولات لحاظ خواهند شد</small>
                        </div>
                        <div class="form-group">
                            <label for="inputPassword3" class="col-sm-4 control-label">دسته‌بندی مربوطه (اختیاری)</label>
                            <select class="form-control" name="categories[]" id="categories" aria-describedby="helpId" multiple>
                                <option value="null">همه دسته‌ها</option>
                                @foreach(\App\Category::all() as $category)
                                    <option value="{{ $category->id }}">{{ $category->name }}</option>
                                @endforeach
                            </select>
                            <small id="helpId" class="text-muted">در صورت خالی بودن، همه دسته‌بندی ها لحاظ خواهند شد</small>
                        </div>
                        <div class="form-group">
