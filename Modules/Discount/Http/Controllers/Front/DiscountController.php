...
class DiscountController extends Controller
{
    public function check(Request $request)
    {
        ...
        $check = UsedDiscount::where([
            ['user_id', auth()->user()->id], ['discount_code', $validated['discount']]
            ] )->first();

        if($check) {
            return back()->withErrors([
                'discount' => 'شما پیشتر از این کد تخفیف استفاده کرده اید'
            ]);
        }
    ...
    public function destroy(Request $request)
    {
        ...
        alert()->success('کد تخفیف اعمال شده، حذف شد.');
        return back();
    }
